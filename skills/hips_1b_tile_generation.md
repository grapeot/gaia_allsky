...
cp skills/hips_landing_page-en.html outputs/hips1b_out_bsc5/index-en.html
```

> reference 存在 **`skills/hips_landing_page*.html`**（进 git）。不要从 `outputs/` 里那份
> 拷——`outputs/` 被 gitignore，且 regenerate HiPS 会覆盖丢失。skills 里的模板才是权威存档。
>
> **落地页文案的唯一来源**：要改 HiPS 落地页的文字（标题/介绍/已知缺陷等），只改
> `skills/` 下的模板。别在 `outputs/.../index.html` 上改（会被覆盖、且 gitignore）。
> 注意保持与主页 `docs/index.html` / `docs/index-en.html` 叙事一致。

当前有两套模板，按要发布的 HiPS 类型选择：

| 用途 | 中文模板 | 英文模板 | 说明 |
|---|---|---|---|
| 全天 Lite / 默认发布版 | `skills/hips_landing_page.html` | `skills/hips_landing_page-en.html` | Gaia DR3 全天 G≤20，Norder7，约 500 亿像素，约 20GB。当前 `tools/render_per_order_pipeline.sh` 自动注入这套。 |
| 广州银心归档版 | `skills/hips_landing_page_guangzhou.html` | `skills/hips_landing_page_guangzhou-en.html` | 旧广州天区 / 银心方向文案，约四百亿像素。若重新发布广州版，手动把这套复制到对应 HiPS 根目录。 |

全天 Lite 当前部署目录是 `outputs/allsky_po_n7_lite/hips/`。上传时部署这个目录的内容即可，
不需要上传 `outputs/allsky_po/` 里的源 TAN tiles 或 per-order 中间产物。完整 Norder8 存档留在
本地 `outputs/allsky_po/hips/`，约 56GB；线上默认用 Norder7 Lite，约 20GB。
...
- 全天 Lite 模板的 survey id 是 `GaiaAllskyLite`，广州归档模板仍用旧 `GaiaMW1B`。
