...
**Norder5 seam/PSF 复查**：用户指出 `Norder5/Dir10000/Npix10566.jpg` 的 PSF 块状异常，并给出三条
N5 seam 坐标：`17:48:09.057 -45:03:21.48`、`17:25:14.033 -43:28:41.8`、`17:37:31.102 -41:53:04.36`。
三个点都落在同 face 的 N5 tile 右边缘附近，距离右边界约 2-7px，右邻 tile 均存在，因此不是覆盖缺失或
跨 face。试了两层最小修复：一是让普通星中心在 tile 外时仍可把 PSF 翼卷回当前 tile，二是把 off-tile
贡献从 Norder(K+9) 整数子像素改为 `cdshealpix` 连续 `dx/dy` 加整数 tile offset。测试通过，但三处 seam
的文件级边缘跳变没有下降；当前可看版本仍以 shear6/half12 为准。另试把 N3-N5 block-local shear 从 64px
降到 16px，`cdshealpix` 在线程池创建上触发 `Resource temporarily unavailable`，该临时参数已撤回。结合
hipsgen 源码调研，剩余 PSF 块状感更可能来自 direct forward splat + 小 sigma 椭圆卷积的采样语义，而
hipsgen 官方路径是每个输出 HEALPix child pixel center 反投影到源图后做 backward bilinear/gather。下一步
应考虑把点源渲染锚到全局 HEALPix child grid 或实现等价的 backward/gather 采样，而不是继续局部调 kernel。
