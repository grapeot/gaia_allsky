...
8. **统一 gamma 输出**：所有 panel 用同一个 signal stretch；`target_white` 以上的高光用软肩平滑滚向显示上限（硬截断会把银心压成无纹理的平台），再做 `gamma=2.2` 输出。
...
已知边界：2D 数据 + 含噪逐星消光的修正能力到此为止。缝/云对比的进一步改进（接近深曝光照片的水平）需要逐方向的消光-距离曲线，即用视差星表构建粗粒度 3D 尘埃模型——它能同时解决薄幕/分布式尘埃的区分和亮星云窗口的方向性增益，列为后续工作。另外注意：深曝光天文照片经过激进的曲线拉伸（压暗影调），与本项目的感知模型有展示口径上的差异，对比时应留有余地。
