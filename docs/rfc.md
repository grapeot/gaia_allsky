...
第二，截断补偿的选星依据。增益按星表固有 G 星等（`g_mag`）选星，亮度用重投影后的视星等（`vis_mag`）计算。「哪些星代理 G>11 不可分辨族群」是恒星身份，不随观测者飞近飞远改变；如果按视星等选星，一颗近处亮星飞远后会被误判成暗星而突然乘上增益，产生亮度跳变。3D 星表实测截断同为 G<11，所以静态图的 `faint_mag_min=9.0 / faint_gain=4.2` 直接沿用。
