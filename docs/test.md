...
所有测试在 `tests/test_render.py`，一个文件，五十多个用例，分三组：不需要数据的自动数学测试、需要本地 Gaia 缓存的条件集成测试、手工视觉验证。
...
- **统一 PSF 星点模型**：饱和溢出守恒总能量、核心截到饱和线、溢出翼获得散布能量；低于饱和线的画布不被改动；截断补偿增益只作用于 G≥faint_mag_min 的暗星；同一 PSF 下亮星经饱和溢出后的可见足迹大于暗星（视尺寸单调性）。
