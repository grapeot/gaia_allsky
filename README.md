...
- **光污染的灵敏度代价**（`render_bortle_eye_grid.py`）：默认生成广州地面竖幅视角的 Bortle 1/6 × 有效灵敏度代价 +0/+2/+4mag 组合对比，输出
  `outputs/knob_bortle_eye_grid.png`。默认 `horizon_window` 是 rectilinear 透视相机：画面下缘经过地平线，水平 FOV 90°，垂直 FOV 75°；默认使用 median sky adaptation，把每个 panel 的背景天空归一到相近灰度，并用高百分位白点压缩避免大片过曝。NELM 不是输入，而是按 Bortle 背景和灵敏度代价计算出来并写入 panel 标签。这张图是正式展示图；SNR 模式只保留作调试/物理 sanity check。
