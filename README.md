...
- **光污染的灵敏度代价**（`render_bortle_eye_grid.py`）：默认生成广州地面竖幅视角的 Bortle 1/6 × 有效灵敏度代价 +0/+2/+4mag 组合对比，输出
  `outputs/knob_bortle_eye_grid.png`。默认 `horizon_window` 是 rectilinear 透视相机：画面下缘经过地平线，水平 FOV 90°，垂直 FOV 75°；默认使用低百分位 sky adaptation，把每个 panel 的背景天空归一到相近深灰，并对背景以上的星光/银河信号做对比度提升，再用高百分位白点压缩避免大片过曝。NELM 不是输入，而是按 Bortle 背景和灵敏度代价计算出来并写入 panel 标签。这张图是正式展示图；SNR 模式只保留作调试/物理 sanity check。
...
两个 CLI 共享同一条 L 型运动轨迹：第一段朝北斗七星方向前进，默认只走约 50pc（约等于旧 400pc 预览的第 68 帧位置，避免飞过北斗）；第二段从当前位置斜向飞到“银心方向上方”的目标点（`银心方向 * target_gc_pc + 银北极方向 * leg2_pc`，默认 `target_gc_pc=400pc`）。
区别只在投影和相机朝向：VR 版输出全天 equirectangular，没有单一镜头方向；前向版默认是满画幅 perspective，第一段看向北斗七星，并用细线连接七颗星，让北斗形状随视差逐渐改变；第二段相机平滑转为 look-at 银盘目标点（默认 `银心方向 * target_gc_pc`），而不是使用太阳系视角下的固定银心方向向量。
